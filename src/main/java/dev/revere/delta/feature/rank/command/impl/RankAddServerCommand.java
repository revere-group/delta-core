...
public class RankAddServerCommand extends BaseCommand {
    ...
    public void onCommand(CommandArgs command) {
        ...
        if (servers.contains(server.getName())) {
            player.sendMessage(CC.translate("&cThe rank &b" + name + " &calready has the server &b" + server.getName() + " &cadded."));
            return;
        }
        ...
        player.sendMessage(CC.translate("&b" + server.getName() + " &fhas been added to the rank &b" + name + "&f."));
    }
}
