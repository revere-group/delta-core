 ...
 * @date 6/23/2024
 */
public class RankRemoveServerCommand extends BaseCommand {
    @Command(name = "rank.removeserver", permission = "delta.rank.removeserver", inGameOnly = true, description = "Remove a server from a rank")
    @Override
    public void onCommand(CommandArgs command) {
        ...
        if (args.length < 2) {
            player.sendMessage(CC.translate("&cUsage: /rank removeserver <name> <server>"));
            return;
        }
